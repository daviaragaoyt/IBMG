...
import {
    Users, ArrowLeft, RefreshCw, Baby, Crown, MapPin, Zap, TrendingUp, Briefcase, Share2, CalendarDays, BarChart2, Activity
} from 'lucide-react';
...
export const DashboardEvento = ({ isLightMode }: { isLightMode: boolean }) => {
    ...
    // Header mais compacto no mobile, mais espaçado no desktop
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 md:gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] md:text-xs font-bold mt-0.5 opacity-60 flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span> {viewMode === 'LIVE' ? 'Ao Vivo' : 'Análise'}</p>
                            </div>
                        </div>
                    </div>

                    {/* Linha 2: Controles Deslizantes (Mobile Friendly) */}
                    <div className="w-full overflow-x-auto pb-1 no-scrollbar flex items-center gap-3">
                        {/* Toggle Live/Analytics */}
                ...
                {viewMode === 'LIVE' ? (
                    ...
                    <div className="animate-fade-in space-y-4 md:space-y-6">
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                            ...
                            <div className="col-span-1 p-4 rounded-[1.5rem] border flex flex-col justify-between h-28 md:h-40" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                                ...
                                <div>
                                    <span className="text-2xl md:text-5xl font-black block" style={{ color: theme.text }}>{accumulatedTotal}</span>
                                    <span className="text-[9px] font-bold opacity-50">Geral</span>
                                </div>
                            ...
                            <div className="col-span-2 lg:col-span-1 p-4 rounded-[1.5rem] border flex flex-col justify-center gap-3 h-auto min-h-[110px]" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                                <div>
                                    <div className="flex justify-between mb-1"><span className="text-[9px] font-bold text-orange-500 uppercase">Visitantes</span><span className="text-xs font-black">{data?.byType?.VISITOR || 0}</span></div>
                                ...
                                <div>
                                    <div className="flex justify-between mb-1"><span className="text-[9px] font-bold text-purple-500 uppercase">Membros</span><span className="text-xs font-black">{data?.byType?.MEMBER || 0}</span></div>
                    ...
                    <div className="animate-fade-in space-y-4 pb-12">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div className="p-5 rounded-[1.5rem] border flex items-center gap-3" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                                <div className="p-3 rounded-full bg-blue-100 text-blue-600"><Activity size={24} /></div>
                                <div><h4 className="text-[10px] font-bold opacity-60 uppercase">Média / Hora</h4><span className="text-2xl font-black block" style={{ color: theme.text }}>{Math.round(todayTotal / 5)}</span></div>
                            </div>
                            <div className="p-5 rounded-[1.5rem] border flex items-center gap-3" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                                <div className="p-3 rounded-full bg-purple-100 text-purple-600"><BarChart2 size={24} /></div>
                                <div><h4 className="text-[10px] font-bold opacity-60 uppercase">Total Geral</h4><span className="text-2xl font-black block" style={{ color: theme.text }}>{accumulatedTotal}</span></div>
                            </div>
                        </div>

                        {/* Gráfico Comparativo */}
                        <div className="p-5 rounded-[1.5rem] border" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                            <h3 className="font-bold text-sm mb-4 flex items-center gap-2"><CalendarDays size={16} className="text-indigo-500" /> Evolução Diária</h3>
                            <div className="h-64 w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={eventDays.map(day => ({ name: `Dia ${day}`, total: data?.checkpointsData?.[day]?.[MAIN_ENTRANCE_NAME]?.total || 0 }))}>
                                        <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={theme.gridColor} />
                                        <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: theme.text, fontSize: 10, fontWeight: 'bold' }} />
                                        <YAxis axisLine={false} tickLine={false} tick={{ fill: theme.subText, fontSize: 10 }} width={25} />
                                        <Tooltip cursor={{ fill: 'transparent' }} contentStyle={CustomTooltipStyle} />
                                        <Bar dataKey="total" fill="#8B5CF6" radius={[6, 6, 0, 0]} barSize={40}>
                                            {eventDays.map((_, index) => (<Cell key={`cell-${index}`} fill={index % 2 === 0 ? '#8B5CF6' : '#A78BFA'} />))}
                                        </Bar>
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                        ...
                        {/* Conversão */}
                        <div className="p-5 rounded-[1.5rem] border" style={{ backgroundColor: theme.cardBg, borderColor: theme.cardBorder }}>
                            <h3 className="font-bold text-sm mb-4">Taxas de Conversão</h3>
                            <div className="flex gap-3">
                                <div className="flex-1 p-3 bg-gray-50 rounded-xl border border-gray-100 text-center">
                                    <span className="text-[9px] font-bold uppercase text-gray-400">Visitantes</span>
                                    <div className="text-xl font-black text-orange-500">{Math.round(((data?.byType?.VISITOR || 0) / (todayTotal || 1)) * 100)}%</div>
                                </div>
                                <div className="flex-1 p-3 bg-gray-50 rounded-xl border border-gray-100 text-center">
                                    <span className="text-[9px] font-bold uppercase text-gray-400">Membros</span>
                                    <div className="text-xl font-black text-purple-600">{Math.round(((data?.byType?.MEMBER || 0) / (todayTotal || 1)) * 100)}%</div>
                                </div>
                            </div>
                        </div>
                    </div>
