...
export const EkklesiaCredential = ({ isLightMode }: EventoProps) => {
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) setUser(await res.json()); else setError('E-mail não encontrado na base de dados.');
    ...
    return (
        <div className="min-h-screen w-full flex flex-col items-center p-6 font-sans transition-colors duration-500"
            ...
            {!user ? (
                <div className="w-full max-w-md p-8 rounded-[2rem] shadow-2xl animate-fade-in relative z-10" style={{ backgroundColor: theme.cardBg }}>
                    <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-6 text-white shadow-lg" style={{ background: theme.gradient }}><Search size={32} /></div>
                    <h2 className="text-2xl font-black text-center mb-2">Buscar Credencial</h2>
                    <p className="text-center opacity-60 mb-6 text-sm">Digite o e-mail que você usou no cadastro.</p>
                    <form onSubmit={handleSearch} className="space-y-4">
                        <input type="email" required placeholder="seu@email.com"
                            className="w-full p-4 rounded-xl border-2 outline-none font-medium transition-all focus:border-[#A800E0]"
                            style={{ backgroundColor: theme.inputBg, borderColor: theme.inputBorder, color: theme.textPrimary }}
                            value={email} onChange={e => setEmail(e.target.value)} />
                        <button disabled={loading} className="w-full py-4 rounded-xl text-white font-bold text-lg shadow-lg hover:brightness-110 disabled:opacity-50" style={{ background: theme.gradient }}>{loading ? 'Buscando...' : 'GERAR ACESSO'}</button>
                    </form>
                ...
                <div className="w-full max-w-sm animate-fade-in relative group z-10">
                    <div className="rounded-[2rem] overflow-hidden shadow-2xl relative" style={{ backgroundColor: theme.cardBg }}>
                        <div className="h-36 relative flex items-center justify-center overflow-hidden" style={{ background: theme.gradient }}>
                            <div className="relative z-10 text-white font-black text-4xl tracking-tighter drop-shadow-lg" style={{ fontFamily: 'Playfair Display' }}>Ekklesia</div>
                        </div>
                        <div className="px-8 pb-10 flex flex-col items-center -mt-12 relative z-10">
                            <div className="bg-white p-4 rounded-2xl shadow-lg mb-6"><QRCode value={user.id} size={180} /></div>
                            <h2 className="text-2xl font-black text-center leading-tight mb-2">{user.name}</h2>
                            <div className={`px-4 py-1 rounded-full text-xs font-bold uppercase tracking-widest mt-2 ${user.type === 'VISITOR' ? 'bg-orange-100 text-orange-600' : 'bg-purple-100 text-purple-600'}`}>{user.type === 'VISITOR' ? 'Visitante' : 'Membro'}</div>
                        </div>
                    </div>
                    <button onClick={() => setUser(null)} className="w-full mt-6 py-3 font-bold text-sm hover:underline" style={{ color: isLightMode ? '#A800E0' : '#FFFFFF' }}>Buscar outro e-mail</button>
                </div>
            )}
        </div>
    );
