...
export const PublicCatalog = ({ type, isLightMode }: any) => {
    ...
    // Tema ajustado para contraste perfeito
    ...
    const handleFinalizeOrder = async (formData: any, proofFile: any, onSuccessCallback: any) => {
        ...
        <div className="min-h-screen font-sans pb-32 transition-colors duration-500 selection:bg-purple-500 selection:text-white" style={{ background: theme.bg, color: theme.text }}>
            ...
            <div className="relative h-72 w-full overflow-hidden z-0 mb-6 group">
                ...
                <div className="absolute top-6 left-6 right-6 flex justify-between z-50">
                    <Link to="/ekklesia" className={`w-10 h-10 flex items-center justify-center backdrop-blur-md rounded-full border shadow-sm transition-all active:scale-90 hover:bg-white/20 ${isLightMode ? 'bg-white/60 text-black border-white/50' : 'bg-black/30 text-white border-white/10'}`}>
                        <ArrowLeft size={20} />
                    </Link>
                    {!isMenuOnly && (
                        <button onClick={() => setIsFlowOpen(true)} className={`relative w-10 h-10 flex items-center justify-center backdrop-blur-md rounded-2xl border shadow-sm transition-all active:scale-90 hover:bg-white/20 ${isLightMode ? 'bg-white/60 text-black border-white/50' : 'bg-black/30 text-white border-white/10'}`}>
                            <ShoppingCart size={20} />
                            {cart.length > 0 && <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center font-bold border-2 border-white/20 animate-bounce">{cart.length}</span>}
                        </button>
                    )}
                </div>

                <div className="absolute inset-0 flex flex-col justify-end pb-10 px-6 max-w-7xl mx-auto z-10 animate-fade-in">
                    <div className={`flex items-center gap-3 mb-3 ${isLightMode ? 'opacity-60 text-black' : 'opacity-80 text-white'}`}>
                        {React.cloneElement(hero.icon, { className: isLightMode ? "text-black opacity-40" : "text-white opacity-40" })}
                        <span className="text-xs font-bold uppercase tracking-[0.3em] border-b border-current pb-1">Ekklesia 2026</span>
                    </div>
            ...
            <div className="relative z-20 px-4 md:px-6 -mt-8 max-w-7xl mx-auto">
                ...
                <div className="backdrop-blur-xl shadow-lg rounded-2xl flex gap-3 border mb-10 transition-all focus-within:ring-2 focus-within:ring-purple-500/50 focus-within:scale-[1.01]"
                    style={{ background: theme.inputBg, borderColor: theme.border, boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.1)' }}>
                    <div className="p-4 pl-5 opacity-40 flex items-center"><Search size={22} /></div>
                    <input
                        type="text"
                        placeholder="O que você procura hoje?"
                        className="flex-1 bg-transparent outline-none font-bold text-lg placeholder:font-normal placeholder:text-gray-400 h-14"
                        style={{ color: theme.text }}
                        value={searchTerm}
                        onChange={e => setSearchTerm(e.target.value)}
                    />
                    {searchTerm && (
                        <button onClick={() => setSearchTerm('')} className="pr-5 opacity-40 hover:opacity-100 transition-opacity">
                            <X size={20} />
                        </button>
                    )}
                </div>
                ...
                {loading ? (
                    ...
                    <>
                        {/* Seção Principal (Ekklesia) */}
                        {ekklesiaCollection.length > 0 && (
                            <div className="mb-12 animate-fade-in">
                                <div className="flex items-center gap-2 mb-5 pl-1">
                                    ...
                                    <h3 className="font-black text-xl uppercase tracking-wider">Destaques Ekklesia</h3>
                                </div>
                                {renderGrid(ekklesiaCollection)}
                            </div>
                        )}

                        {/* Seção Secundária (Outros) */}
                        {otherCollection.length > 0 && (
                            <div className="animate-fade-in">
                                <div className="flex items-center gap-2 mb-5 pl-1 opacity-70">
                                    <Filter size={18} />
                                    <h3 className="font-bold text-sm uppercase tracking-wider">Todos os Produtos</h3>
                                </div>
                                {renderGrid(otherCollection)}
                            </div>
                        )}

                        {/* Empty State */}
                        {allProducts.length === 0 && (
                            <div className="text-center py-20 opacity-50 animate-fade-in flex flex-col items-center">
            ...
            {/* TOASTS (Notificações) */}
            <div className="fixed top-6 right-6 flex flex-col items-end pointer-events-none z-[100] gap-2">
                {toasts.map(t => <Toast key={t.id} msg={t.msg} type="success" />)}
            </div>
            ...
            {!isMenuOnly && (
                <FlowModal isOpen={isFlowOpen} onClose={() => setIsFlowOpen(false)} cart={cart} setCart={setCart} total={cart.reduce((a, b) => a + b.price * b.quantity, 0)} onConfirm={handleFinalizeOrder} loading={checkoutLoading} />
            )}
                ...
                <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 animate-fade-in">
                    ...
                    <div className="relative w-full max-w-sm bg-white p-8 rounded-[2.5rem] shadow-2xl text-center animate-scale-up border border-white/10">
                        ...
                        <div className="bg-gray-50 p-6 rounded-3xl mb-8 border border-gray-100">
                            <p className="text-xs font-bold uppercase opacity-40 mb-2 tracking-widest text-gray-900">Seu Código de Retirada</p>
                            <p className="text-6xl font-black tracking-widest text-purple-600 font-mono">#{successOrder?.orderCode}</p>
                        </div>

                        <button onClick={() => setSuccessOrder(null)} className="w-full py-4 bg-gray-900 text-white rounded-2xl font-bold uppercase active:scale-95 transition-transform shadow-xl">
                            Fechar e Comprar Mais
                        </button>
                    </div>
                </div>
            )}
