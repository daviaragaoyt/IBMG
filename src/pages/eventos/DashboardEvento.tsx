...
export const DashboardEvento = ({ isLightMode, data }: { isLightMode: boolean, data?: any }) => {
    ...
    const daysToShow = useMemo(() => {
        if (!localData?.checkpointsData) return [today]; // Fallback se não houver dados

        // Pega as chaves do objeto (ex: "11", "12") que vieram do banco
        const apiDays = Object.keys(localData.checkpointsData);

        // Junta com o dia de hoje e ordena
        return Array.from(new Set([today, ...apiDays]))
            .sort((a, b) => parseInt(a) - parseInt(b));
    }, [today, localData]);

    // Lógica de Estatísticas (Mantida)
    ...
    const hourlyData = localData?.timeline?.[selectedDay] ? Object.keys(localData.timeline[selectedDay]).sort((a, b) => parseInt(a) - parseInt(b)).map(h => ({ name: `${h}h`, value: localData.timeline[selectedDay][h] })) : [];
    ...
    return (
        <div className={`w-full ${!isLightMode ? 'dark' : ''}`}>
            ...
            {/* CSS INJETADO PARA ESCONDER SCROLLBAR */}
            <style>{`
            ...
            <div className="w-full py-4 animate-fade-in">
                ...
                {/* MENU DE ABAS + SELETOR DE DIAS (LINHA ÚNICA) */}
                ...
                {/* --- CONTEÚDO DAS ABAS --- */}
                {/* ... (Todo o restante do código permanece igual, focando na renderização dos cards) ... */}
                ...
                {activeTab === 'PEOPLE' && (
                    <div className="space-y-6 animate-slide-up">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <Card className="h-[400px]"><CardTitle icon={<UserCheck size={18} />} title={`Idade (Dia ${selectedDay})`} color="indigo" /><ResponsiveContainer width="100%" height="100%"><BarChart data={ageData} layout="vertical"><XAxis type="number" hide /><YAxis dataKey="name" type="category" width={80} tick={{ fill: chartTheme.text, fontSize: 11, fontWeight: 'bold' }} axisLine={false} tickLine={false} /><Tooltip cursor={{ fill: 'transparent' }} contentStyle={{ borderRadius: '12px', border: `1px solid ${chartTheme.tooltipBorder}`, backgroundColor: chartTheme.tooltipBg, color: chartTheme.color }} /><Bar dataKey="value" barSize={30} radius={[0, 10, 10, 0] as any}>{ageData.map((e, i) => <Cell key={i} fill={e.fill} />)}</Bar></BarChart></ResponsiveContainer></Card>
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'MARKETING' && (
                    <div className="space-y-6 animate-slide-up">
                        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                            ...
                            <Card className="xl:col-span-2 h-80 !items-stretch !p-8"><div className="flex items-center gap-3 mb-4"><Layers className="text-purple-500" /><h3 className="font-bold text-lg">Evolução Geral (Todo o Evento)</h3></div><div className="flex-1 min-h-0"><ResponsiveContainer width="100%" height="100%"><ComposedChart data={evolutionData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke={chartTheme.grid} /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 12, fontWeight: 'bold', fill: chartTheme.text }} dy={10} /><Tooltip contentStyle={{ borderRadius: '12px', border: `1px solid ${chartTheme.tooltipBorder}`, backgroundColor: chartTheme.tooltipBg, color: chartTheme.color }} /><Bar dataKey="total" barSize={50} fill={COLORS.marketing} radius={[8, 8, 0, 0] as any} /><Line type="monotone" dataKey="visitantes" stroke={COLORS.visitor} strokeWidth={4} dot={{ r: 4, strokeWidth: 2, fill: 'white' }} /></ComposedChart></ResponsiveContainer></div></Card>
                        </div>
                    </div>
                )}
