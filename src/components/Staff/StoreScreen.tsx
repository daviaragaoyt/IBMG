...
import { StoreOrders } from '../StoreOrders'; // <--- THE NEW COMPONENT
...
export const StoreScreen = ({ user, checkpoints, selectedSpot, setSelectedSpot, addToast, onLogout, theme }: any) => {
    ...
    const [_, setProofFile] = useState<File | null>(null);
    ...
    const handleFinishSale = async () => {
        ...
        setLoading(true);
        try {
            // 👇 CORREÇÃO: Enviando JSON simples em vez de FormData
            // Isso garante que o backend consiga ler os dados corretamente
            const payload = {
                name: buyerName || 'Balcão', // Mapeia para 'name' que o backend espera
                buyerType: 'VISITOR',
                manualType: 'VISITOR', // Força o tipo para garantir
                total: cart.reduce((a, b) => a + b.price * b.quantity, 0),
                paymentMethod: paymentMethod, // 'DINHEIRO', 'CARTAO', etc.
                items: cart.map(i => ({
                    productId: i.id,
                    name: i.name,
                    quantity: i.quantity,
            ...
            // Se você precisar enviar foto no futuro, precisaremos configurar o Multer no backend.
            // Por enquanto, enviamos sem foto para a venda passar.
    ...
    return (
        <ScreenLayout user={user} title={isCantina ? "Cantina" : "Store"} icon={<Icon />} accentColor={accentColor} onLogout={onLogout} checkpoints={checkpoints} selectedSpot={selectedSpot} setSelectedSpot={setSelectedSpot} theme={theme}>
            <div className="flex bg-white/5 p-1 mx-4 mt-2 rounded-xl border border-white/10 shrink-0">
                <button onClick={() => setMode('POS')} className={`flex-1 py-3 rounded-lg text-xs font-black transition-all flex items-center justify-center gap-2 ${mode === 'POS' ? 'bg-white text-gray-900 shadow-lg' : 'text-gray-500 hover:bg-white/5'}`}><ShoppingBag size={16} /> NOVA VENDA</button>
                <button onClick={() => setMode('ORDERS')} className={`flex-1 py-3 rounded-lg text-xs font-black transition-all flex items-center justify-center gap-2 ${mode === 'ORDERS' ? 'bg-white text-gray-900 shadow-lg' : 'text-gray-500 hover:bg-white/5'}`}><LayoutDashboard size={16} /> PEDIDOS / DELIVERY</button>
            </div>
                        ...
                        {cart.length > 0 && (
                            <div className="absolute bottom-0 left-0 w-full bg-white dark:bg-gray-900 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] rounded-t-[2rem] p-6 z-30 border-t border-gray-800 animate-slide-up">
                                <div className="flex justify-between items-center">
                                    <div><p className="text-xs font-bold text-gray-400 uppercase">Total a Receber</p><h2 className="text-3xl font-black" style={{ color: accentColor }}>{formatCurrency(cart.reduce((acc, i) => acc + i.price * i.quantity, 0))}</h2></div>
                        ...
                        {isCheckout && (
                            <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4 animate-fade-in">
                                <div className="bg-white dark:bg-gray-900 w-full max-w-sm rounded-t-[2.5rem] md:rounded-[2.5rem] p-8 shadow-2xl animate-slide-up border border-white/10">
                                    <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-black text-gray-800 dark:text-white flex items-center gap-2"><DollarSign className="text-green-500" /> Pagamento Balcão</h3><button onClick={() => setIsCheckout(false)} className="p-2 bg-gray-100 dark:bg-gray-800 rounded-full text-gray-500"><XCircle size={20} /></button></div>
                                    <input
                                        type="text"
                                    ...
                                    />
                                    <div className="grid grid-cols-3 gap-3 mb-6">{['DINHEIRO', 'PIX', 'CARTAO'].map(m => (<button key={m} onClick={() => setPaymentMethod(m)} className={`p-3 rounded-xl border-2 text-[10px] font-bold transition-all ${paymentMethod === m ? 'border-green-500 bg-green-500/10 text-green-500 shadow-lg' : 'border-gray-200 dark:border-gray-700 text-gray-400'}`}>{m}</button>))}</div>
                                    <button onClick={handleFinishSale} disabled={!paymentMethod || loading} className="w-full py-4 bg-green-600 text-white rounded-xl font-black shadow-lg disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 transition-all text-lg">{loading ? <RefreshCw className="animate-spin mx-auto" /> : 'CONFIRMAR VENDA'}</button>
                                </div>
                            </div>
                        )}
                    </div>
